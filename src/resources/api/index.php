...
function updateResource($db, $data) {
    ...
    // check existence
    $chk = $db->prepare("SELECT id FROM resources WHERE id = :id");
...
function deleteResource($db, $resourceId) {
    ...
    try {
        ...
        // delete comments (if your DB has ON DELETE CASCADE this is optional, but safe)
        $stmtC = $db->prepare("DELETE FROM comments WHERE resource_id = :id");
        $stmtC->bindValue(':id', $id, PDO::PARAM_INT);
        $stmtC->execute();

        // delete resource
        $stmtR = $db->prepare("DELETE FROM resources WHERE id = :id");
        $stmtR->bindValue(':id', $id, PDO::PARAM_INT);
        $stmtR->execute();
...
function createComment($db, $data) {
    ...
    $rid = (int)$data['resource_id'];

    // ensure resource exists
    $chk = $db->prepare("SELECT id FROM resources WHERE id = :id");
    $chk->bindValue(':id', $rid, PDO::PARAM_INT);
    $chk->execute();
    if (!$chk->fetch(PDO::FETCH_ASSOC)) {
        sendResponse(['success' => false, 'message' => 'Resource not found'], 404);
    }
    ...
    $sql = "INSERT INTO comments (resource_id, author, text) VALUES (:rid, :author, :text)";
    $stmt = $db->prepare($sql);
...
function deleteComment($db, $commentId) {
    ...
    $chk->bindValue(':id', $cid, PDO::PARAM_INT);
    $chk->execute();
    if (!$chk->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($method === 'GET') {
        ...
        // Call deleteResource()
         if ($action === 'delete_comment') {
            // comment id may be in query param or body
            $cid = $comment_id ?: (isset($body['comment_id']) ? $body['comment_id'] : null);
            deleteComment($db, $cid);
        } else {
            // resource id may be in query param or body
            $rid = $idParam ?: (isset($body['id']) ? $body['id'] : null);
